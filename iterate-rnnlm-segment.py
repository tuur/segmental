...
if __name__ == '__main__':
    ...
    subprocess.call(['python2.7',segmenter,args.text,'-threshold',str(args.threshold),'-hidden',str(args.hidden), '-output',args.output+'/seg-1.txt', '-fast', str(args.fast)])

    for i in range(1,args.it):
        sys.stderr.write('--- ITERATION '+str(i+1)+':\n')
        inp = args.output+'/seg-'+str(i)+'.txt'
        outp = args.output+'/seg-'+str(i+1)+'.txt'
        subprocess.call(['python2.7',segmenter,inp,'-threshold',str(args.threshold), '-hidden',str(args.hidden), '-output', outp, '-fast', str(args.fast)])
    if args.clean:
